...
$result_rs= ProductDBRent::getProductsVip();
$vip = count($result_rs);
foreach ($result_rs as $key_rrs => $value_rrs) {
    if ($key_rrs < 8) {
        ...
        $data_product = array(
            ...
            'image' => $img,
            'land_area' => $land_area,
            'estimated_price' => $result.'/tháng',
            'post_time' => $time,
            'kind_id'=>$value_rrs['kind_id'],
            'kind_new' => $kind_new['name']
        );
        array_push($data['rrs'], $data_product);
    }
}
if ($vip < 8){
    $result_rs2 = ProductDBRent::getProductsEndow();
    ...
    foreach ($result_rs2 as $key_rrs => $value_rrs) {
        if ($key_rrs < (8-$vip)) {
            ...
            array_push($data['rrs'], $data_product2);
        }
    }
    if ($endow < 8) {
        $result_rs1 = ProductDBRent::getProductsOften();
